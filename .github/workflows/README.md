...
Configuration for a pipeline that builds docker images for changed application folders, pushes them to Docker Hub and sends a workflow dispatch to the [manifests repository](https://github.com/hinichijou/DevOpswithKubernetesManifests/tree/5.1/.github/workflows).
...
* The dummysite app is built on changes to the `dummysite_app` folder and dummysite controller on changes to the `dummysite_controller` folder. The images are named: *image-name*:*commit-sha*. Built only on push to main branch.