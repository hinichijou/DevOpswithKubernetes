...
Application that displays a list of todos and a random image from `https://picsum.photos/1200`. Consists of a React + Vite frontend and a Node.js + Hono backend.
...
Deploy with `kubectl apply -f manifests`. This creates deployment, ingress and service resources defined by the yamls in the manifests folder. The frontend service connects the application port to a cluster internal network port and the ingress routes all traffic to this service port. The frontend routes requests to `/api` to the backend service.
...
You can view the HTML page served from http://localhost:8081. The backend fetches a new image from https://picsum.photos/1200 every 10 minutes. The backend writes the image to persistent storage so it persists between application restarts. Frontend reads said image from the persistent storage and displays it.
